...
histo <- hist(Airbnb_Data$log_price, plot = FALSE)
histo2 <- hist(Airbnb_Data$bedrooms, plot = FALSE, breaks = 4)
...
Airbnb_Data <- Airbnb_Data |> 
  mutate(clase_price = cut(log_price, breaks = histo$breaks)) |> 
  mutate(clase_bedrooms = cut(bedrooms, breaks = histo2$breaks))
# Ajusta manualmente los niveles de la variable 'clase_ph' para incluir un intervalo específico
levels(Airbnb_Data$clase_price) <- c(levels(Airbnb_Data$clase_bedrooms))

# Crea una tabla de contingencia entre las categorías de 'ph' y 'est', añadiendo totales con addmargins(), y la muestra
tabla <- table(Airbnb_Data$clase_price, Airbnb_Data$clase_bedrooms) |> addmargins()
tabla |> kable()
