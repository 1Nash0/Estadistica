...
datos <- Airbnb_Data$log_price  
 ggplot(data.frame(x = datos), aes(x = x)) +
     geom_histogram(binwidth = 50, fill = "#F16E79", color = "black", 
                    ) +
...
Como podemos observar en este histograma la mayor parte de los precios se encuentran entre 1\$ y 250\$.
...
Igual que hemos visto en el histograma, el resumen numérico nos demuestra que la mayor parte de los precios se concentran entre 1\$ y los 185\$, el precio mínimo es de 1\$ y el máximo es de 1999\$, aunque en muy pocos casos se tienen estos valores, y la media de los precios se encuenta en los 160.37\$
...
De la misma forma realizaremos una gáfica con los datos en función de la frecuencia absoluta.
...
Estas gáficas nos muestran que la mayoría de ofertas nos ofrecen la totalidad de tipo de propiedad que alquilamos con más del 50% de ofertas, seguidas por las ofertas que nos ofrecen una habitación privada con un poco más del 40% del total y ya por último en menor cantidad nos encontramos las ofertas que ofrecen una habitación compartida con menos del 5% del total de ofertas.
...
Podemos observar que se usan decimales para hablar del número de baños, esto se debe a que para que se pueda consider un baño completo tiene que tener un lavabo, un inodoro, una ducha o bañera y un mueble de almacenamiento. El gáfico nos premite ver que la mayoría de ofertas solo tiene un baño, el mínimo de baños es 0 y el máximo son 8, y la media de baños que se ofertan son 1.25 baños.
...
Aquí estudiaremos cuales son los tipos de habitaciones que más se ofertan.
...
```{r}
Airbnb_Data  |> group_by(neighbourhood) |>  summarise(Frequency = n()) |> mutate(neighbourhood = reorder(neighbourhood, -Frequency)) |> ggplot(aes(x = neighbourhood, y = Frequency)) + geom_bar(stat = "identity", aes(fill = Frequency)) + theme_bw() + labs(title = "Gráfico de barras con frecuencia relativa de neighbourhood", x = "Barrios", y = "Frecuencia absoluta") + scale_fill_gradient("Frecuencia", low = "#F16E79", high = "red") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

Como podemos observar hay tantos datos que el análisis es ilegibles
