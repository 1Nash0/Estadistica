...
Estudiamos las frecuencias relativas y absolutas con la tabla de frecuencias.

```{r}
intervalos <- seq(0, ceiling(max(Airbnb_Data$number_of_reviews)/20) * 20, by = 20)
tabla_frecuencias_number_of_reviews <- Airbnb_Data |> count(number_of_reviews) |> mutate(f = n / nrow(Airbnb_Data), N = cumsum(n), F = cumsum(f))
 ...
 tabla_frecuencias_ordenada1 <- tabla_frecuencias_number_of_reviews |> arrange(Intervalo)
 print(tabla_frecuencias_ordenada1)
```
...
CONTAR LA PUNTUACIÓN DE REVIEWS
...
```{r}
tabla_frecuencias_review_scores_rating <- Airbnb_Data |> count(review_scores_rating) |> mutate(f = n / nrow(Airbnb_Data), N = cumsum(n), F = cumsum(f)) 
tabla_frecuencias_ordenada5 <- tabla_frecuencias_review_scores_rating |>
    arrange(desc(n))
 tabla_frecuencias_ordenada5$F <- cumsum(tabla_frecuencias_ordenada5$f)
 tabla_frecuencias_ordenada5$N <- cumsum(tabla_frecuencias_ordenada5$n)
 print(tabla_frecuencias_ordenada5)
...
Observando la gráfica podemos darnos cuenta de que esta y el valor obtenido en el estudio de correlación numérica son coherentes entre sí. Podemos ver en la gráfica que hay un amento general del precio de la propiedad según los baños que haya. No obstante, hay algunos datos atípicos que provocan un descenso en el coeficiente de correlación lineal, pero aun así se puede identificar de manera correcta el aumento del precio de la propiedad por baño.
...
Podemos observar que la correlación entre el precio y el resto de variables no es muy elevado, y es muy parecido en todas, en cambio podemos observar como hay una mayor correlación entre el número de habitaciones y el número de baños y sobre todo entre el número de habitaciones y el número de camas. La relación entre el número de habitaciones y el de camas, nos ayuda a ver que cuantas más habitaciones, más personas pueden alojarse en la casa, ya que aumenta también el número de camas y no suelen ser habitaciones "vacías".
...
Como podemos ver, la gráfica posee una regresión lineal esperable, a cuantas más habitaciones haya, mayor es el número de baños, y se estabiliza en los 5 baños, a pesar de la gran cantidad de valores atípicos que hay.
...
Aun así, debido al aumento de habitaciones respecto a los baños, podemos deducir que cuantos ms baños/habitaciones haya, más metros cuadrados tiene la propiedad. Con esto, y habiendo visto anteriormente que la variable del precio aumenta según sube la cantidad de baños, podemos relacionar que también el precio aumenta según el tamaño del área de la propiedad. Además, de que cuantas más habitaciones, mayor es el precio. Esto significa que, gracias al haber visto anteriormente comparando las variables de las habitaciones con las del número de camas, cuantas ms personas se puedan alojar, mayor será el precio de la propiedad.
...
De manera general, se puede observar que las reseñas con mejor puntaje tienden a acumularse en las opciones más económicas. Este cúmulo se percibe mejor como el area bajo la curva que forman los puntos.
...
A pesar de ello, se identificaron varias variables con una buena correlación lineal entre sí. La relación más notable fue entre las variables price y review_score_rating, donde se observó que las calificaciones de las reseñas dependen más del precio que viceversa. También se pudo concluir que, efectivamente, el precio aumenta según aumenta la gente que pueda alojarse en la propiedad y según aumentan los metros cuadrados de la misma, teniéndolo muy en cuenta a la hora de reservar en esta plataforma.

Cabe comentar que el estudio se ha visto ligeramente afectado negativamente debido la presencia de un algunos datos atípicos, lo que ha impactado en la calidad de los resultados obtenidos, pero que aun así se han logrado obtener unas respuestas válidas para el estudio.
