...
```{r}
Airbnb_Data |> ggplot(aes(log_price)) +
  geom_histogram(
    fill = "steelblue",  
    bins = 10,           
    col = "white"       
...
```{r}
Airbnb_Data  |> group_by(neighbourhood) |>  summarise(Frequency = n()) |> mutate(neighbourhood = reorder(neighbourhood, -Frequency)) |> ggplot(aes(x = neighbourhood, y = Frequency)) + geom_bar(stat = "identity", aes(fill = Frequency)) + theme_bw() + labs(title = "Gráfico de barras con frecuencia relativa de neighbourhood", x = "Barrios", y = "Frecuencia absoluta") + scale_fill_gradient("Frecuencia", low = "skyblue", high = "#CB0019") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
```{r}
hist(Airbnb_Data$review_scores_rating, col = "#CB0019", main = "Histograma de Puntuación de reviews", xlab = "review_scores_rating", ylab = "Frecuencia", border = "black")
```
...
```{r}
Airbnb_Data |> ggplot(aes(bedrooms)) +
  geom_histogram(
    fill = "steelblue",  
    bins = 11,           
    col = "white"       
