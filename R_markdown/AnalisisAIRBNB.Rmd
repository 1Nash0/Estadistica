...
```{r}
table(Airbnb_Data$instant_bookable)
Airbnb_Data |> 
  ggplot(aes(Airbnb_Data$instant_bookable)) +
  geom_bar(fill = "#CB0019") 
```
...
cat("El número de respuestas obtenido es: ", num_respuestas_number_of_reviews)
```
...
```{r}
Airbnb_Data  |> group_by(review_scores_rating) |>  summarise(Frequency = n()) |> mutate(Frequency = Frequency/sum(Frequency), review_scores_rating = reorder(review_scores_rating, -Frequency)) |> ggplot(aes(x = review_scores_rating, y = Frequency)) + geom_bar(stat = "identity", aes(fill = Frequency)) + scale_y_continuous(labels = scales::percent) + theme_bw() + labs(title = "Gráfico de barras con frecuencia relativa de reseñas", x = "Valoraciones", y = "Frecuencia relativa") + scale_fill_gradient("Frecuencia", low = "skyblue", high = "#CB0019") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
