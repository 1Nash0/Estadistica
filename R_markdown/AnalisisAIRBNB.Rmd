...
Durante estos últimos años, plataformas donde de reservan propiedades durante un tiempo determinado, como es Airbnb, ha ido subiendo su popularidad y mucha gente opta por este tipo de servicio. Es por ello que un análisis de este modelo de negocio es interesante tanto para los clientes como para la empresa propietaria de Airbnb. Un
...
Airbnb_Data <- read.csv("~/ESTADISTICA/Practica Grupal/AnalisisAIRBNB/Airbnb_Data.csv")
```
...
-   `host_identity_verified`: Si el anfitrión est verificado en Airbnb

-   `instant_bookable`: Si se puede reservar la propiedad al instante

-   `number_of_reviews`: Numero de valoraciones de la propiedad
...
```{r HistoPrice}
hist(Airbnb_Data$log_price, col = "#CB0019", main = "Histograma de Precio", xlab = "Price", ylab = "Frecuencia", ylim = c(0, 35000), xlim = c(0,1000), border = "black")
```
...
HISTOGRAMA SEGUN NUMERO DE BAÑOS
...
ggplot(data.frame(x = datos), aes(x = x)) +
     ...
     stat_function(fun = dnorm, args = list(mean = mean(datos),  sd = sd(datos)), color = "red", size = 1.5) +
...
```{r}
um_respuestas_cancellation_policy <- length(unique(Airbnb_Data$cancellation_policy)) 
...
```{r}
Airbnb_Data  |> group_by(cleaning_fee) |>  summarise(Frequency = n()) |> mutate(Frequency = Frequency/sum(Frequency), cleaning_fee = reorder(cleaning_fee, -Frequency)) |> ggplot(aes(x = cleaning_fee, y = Frequency)) + geom_bar(stat = "identity", aes(fill = Frequency)) + scale_y_continuous(labels = scales::percent) + theme_bw() + labs(title = "Gráfico de barras con frecuencia relativa de cleaning_fee", x = "Impuesto de Liempieza", y = "Frecuencia relativa") + scale_fill_gradient("Frecuencia", low = "blue", high = "skyblue") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
```{r}
Airbnb_Data  |> group_by(city) |>  summarise(Frequency = n()) |> mutate(Frequency = Frequency/sum(Frequency), city = reorder(city, -Frequency)) |> ggplot(aes(x = city, y = Frequency)) + geom_bar(stat = "identity", aes(fill = Frequency)) + scale_y_continuous(labels = scales::percent) + theme_bw() + labs(title = "Gráfico de barras con frecuencia relativa de cleaning_fee", x = "Impuesto de Liempieza", y = "Frecuencia relativa") + scale_fill_gradient("Frecuencia", low = "#B1B100", high = "yellow") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
Airbnb_Data |> 
  ggplot(aes(Airbnb_Data$host_identity_verified)) +
...
```{r}
Airbnb_Data  |> group_by(neighbourhood) |>  summarise(Frequency = n()) |> mutate(Frequency = Frequency/sum(Frequency), neighbourhood = reorder(neighbourhood, -Frequency)) |> ggplot(aes(x = neighbourhood, y = Frequency)) + geom_bar(stat = "identity", aes(fill = Frequency)) + scale_y_continuous(labels = scales::percent) + theme_bw() + labs(title = "Gráfico de barras con frecuencia relativa de neighbourhood", x = "¿Se puede reservar al instante?", y = "Frecuencia relativa") + scale_fill_gradient("Frecuencia", low = "#B1B100", high = "yellow") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
```{r}
Airbnb_Data  |> group_by(review_scores_rating) |>  summarise(Frequency = n()) |> mutate(Frequency = Frequency/sum(Frequency), review_scores_rating = reorder(review_scores_rating, -Frequency)) |> ggplot(aes(x = review_scores_rating, y = Frequency)) + geom_bar(stat = "identity", aes(fill = Frequency)) + scale_y_continuous(labels = scales::percent) + theme_bw() + labs(title = "Gráfico de barras con frecuencia relativa de reseñas", x = "Tipo de casa", y = "Frecuencia relativa") + scale_fill_gradient("Frecuencia", low = "skyblue", high = "#CB0017") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
#### Estudio de las variables x e y
